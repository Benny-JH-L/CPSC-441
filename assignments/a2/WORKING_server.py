...
def find_memes_folder(start_path="."):
    '''
    Finds the a path to the `MEME_FOLDER_NAME`
    '''
...
def get_images_paths_from_folder(folder_path):
    '''
    Returns a list of paths to all the memes in the `MEME_FOLDER_NAME`
    '''
...
def get_image_names_from_folder(folder_path):
    '''
    Returns a list of file names of the memes in the `MEME_FOLDER_NAME`
    '''
    image_extensions = {".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp"}
    return [
...
def handle_client(client_socket):
    ...
    try:
        ...
        # check if the url is the easter egg url
        if url == EASTER_EGG_URL:
            response = get_easter_egg_surpise()
        ...
        # otherwise process response
            ...
            while True:
                ...
                buffer += response  # add the chunk 
            
            list_of_responses = inject(buffer)  # inject memes where possible
...
def get_easter_egg_surpise():
    '''
    turns the whole web page into a meme.
    '''
    print("\nEntered easter egg\n")
    
    # get a randome meme from memes folder
    ...
    easter_egg_response = (
        b'HTTP/1.1 200 OK\r\n'
        b'Content-Type: ' + memeExtention.encode() + b'\r\n'
        b'Content-Length: ' + str(length_of_meme).encode() + b'\r\n'
        b'\r\n'
    )
    
    easter_egg_response += memeData
...
def inject(message):
    ...
    if message.find(b'HTTP/') == -1:
        print("\nweb server response does not contain HTTP/... not going to inject\n")
        return [message]
    ...
    while message != b'':
        ...
        # Case where the position of `Content-Length:` is < the position of `Content-Type:`
        if (pos_content_length < pos_content_type):
            ...
            new_message += message[:pos_content_length] + newContentLength       # Add what ever was before `Content-Length`, then replace the old content-length with the meme's content-length, and add it to the new response
            # print("\nreplaced content-length(new message is now):\n",new_message) # debug
            
            new_message += message[pos_content_length_rn:pos_content_type] + newContentType        # Add what ever was after `Content-Length` but before `Content-Type` then replace the old content type with new content type and add it to the new response
            ...
            new_message += message[pos_content_type_rn:pos_header_data_separator] # add any other header data after `content-type` to the new message that are before the separator '\r\n\r\n`
            # print("\nAdding other:\n",new_message)
            
            print("entered case 1, content type length < content type pos") # debug
        ...
        # Case where the position of `Content-Length:` is > the position of `Content-Type:`   
            ...
            # print("position of \\r\\n after `content-length is:", pos_content_length_rn, "\n\tStarting char:", repr(message[pos_content_length_rn]))
            new_message += message[:pos_content_type] + newContentType      # Add what ever was before `Content-Type`, then replace the old content-type with the meme's content-type, and add it to the new response
            # print("\nreplaced content-length(new message is now):\n",new_message) # debug
            
            new_message += message[pos_content_type_rn:pos_content_length] + newContentLength        # Add what ever was after `Content-Type` but before `Content-length` then replace the old content length with new content type and add it to the new response
            # print("\nreplaced content-type (new message is now):\n",new_message) # debug
            ...
            new_message += message[pos_content_length_rn:pos_header_data_separator] # add any other header data after `content-length` to the new message that are before the separator '\r\n\r\n`
            # print("\nAdding other:\n",new_message)
            
            print("entered case 2, content type pos < content length pos") # debug
        ...
        print("\nNew message created (not including data in this print):\n",new_message)     # debug
        ...
        if (pos_HTTP != -1):    
            message = message[pos_HTTP:]    # copies everything starting from the next instance of 'HTTP'
